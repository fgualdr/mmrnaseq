...
def main(**kwargs):
    ...
    if args is None: ## i.e. standalone script called from command line in normal way
        ...
        parser.add_argument("-r","--rmsk", help = "Repeatmasker file (NOT optional; it has to be provided)", type=str, metavar = "<rmsk.txt or file.out i.e. the out of the repeatmasker>")
    ...
    # rmks is unzipped before in the nextflow process so rmks is always a file either rmsk.txt or .out
    
    if not rmsk:
        raise Exception("No Repeatmasker file provided")
    ...
    if RM_out:
        ...
        build=rmsk_name.replace(".rmsk.txt","")
